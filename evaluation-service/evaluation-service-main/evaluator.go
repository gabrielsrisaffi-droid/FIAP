...
func (a *App) getCombinedFlagInfo(flagName string) (*CombinedFlagInfo, error) {
	...
	if err == nil {
		if err := a.RedisClient.Set(ctx, cacheKey, jsonData, CACHE_TTL).Err(); err != nil {
			log.Printf("Erro ao atualizar cache da flag '%s': %v", flagName, err)
		}
	}
...
func (a *App) fetchFlag(flagName string) (*Flag, error) {
	...
	defer func() {
		if err := resp.Body.Close(); err != nil {
			log.Printf("Erro ao fechar resposta do flag-service: %v", err)
		}
	}()
...
func (a *App) fetchRule(flagName string) (*TargetingRule, error) {
	...
	defer func() {
		if err := resp.Body.Close(); err != nil {
			log.Printf("Erro ao fechar resposta do targeting-service: %v", err)
		}
	}()
